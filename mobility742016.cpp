...
geometry_msgs::Pose2D returnPatternPosition;
...
int main(int argc, char **argv) {
  ...
  generatePattern(4,MAX_NUM_ROBOTS); //circuit,robots
  ros::spin();
  //printPattern();
  return EXIT_SUCCESS;
}
  ...
  if (currentMode == 2 || currentMode == 3) { //Robot is in automode
    ...
    case STATE_MACHINE_TRANSFORM: {
      ...
      //If returning with a target
          ...
          returnPatternPosition.x = goalLocation.x;
          returnPatternPosition.y = goalLocation.y;
          returnPatternPosition.theta = returnPatternPosition.theta;
      ...
      //Otherwise, reset target and set new goal location
      else { 

          getTarget();
         // goalLocation.theta = returnPatternPosition.theta;
          goalLocation.x = returnPatternPosition.x;
          goalLocation.y = returnPatternPosition.y;
...
void setVelocity(double linearVel, double angularVel) 
{
    ...
    velocity.linear.x = linearVel * .5;
    velocity.angular.z = angularVel * 4; //scaling factor for sim; removed by aBridge node
...
void targetHandler(const shared_messages::TagsImage::ConstPtr& message) {
    ...
    if (message->tags.data[0] == 256) {
        ...
        if (targetDetected.data != -1) {
            ...
            //attempt to set goalLocation to spot found food after finishing dropping food
    ...
    //if target has not previously been detected 
        ...
        //saving current location and next goal location in spiral when food is detected
        // returnPosition.x = currentLocation.x;
        // returnPosition.y = currentLocation.y;
        ...
        if (!targetsCollected[message->tags.data[0]]) {
            ...
            ROS_ERROR_STREAM("LT targetPosition: " << currentLocation.x << "," << currentLocation.y);

            setHeadingToNest();  

            //set goalLocation to next goal in pattern after picked up target
            // goalLocation.x = returnPatternPosition.x;
            // goalLocation.y = returnPatternPosition.y;
